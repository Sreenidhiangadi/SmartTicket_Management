...
public class UserServiceImpl implements UserService {
	...
	public Mono<UserResponse> getById(String id) {
		return repository.findById(id).switchIfEmpty(Mono.error(new NotFoundException("User not found")))
	...
	public Flux<UserResponse> getActiveAgents() {
		return repository.findByRolesContainingAndActive("AGENT", true).map(UserResponse::from);
	}
