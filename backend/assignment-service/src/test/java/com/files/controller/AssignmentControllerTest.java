...
class AssignmentControllerTest {
    ...
    void assignTicket() {
        ...
        when(assignmentService.assignTicket(any(), any(), any()))
    ...
    @Test
    void autoAssign() {
        AutoAssignmentResponse resp =
                AutoAssignmentResponse.builder()
                        .ticketId("t1")
                        .agentId("a1")
                        .priority("MEDIUM")
                        .slaDueAt(Instant.now())
                        .build();

        when(assignmentService.autoAssign("t1", null))
                .thenReturn(Mono.just(resp));

        webTestClient
                .post()
                .uri("/api/assign/auto/t1")
                .exchange()
                .expectStatus().isOk();
    }
    ...
    void escalationsByManager() {
        ...
        webTestClient
                .mutateWith(SecurityMockServerConfigurers.mockJwt())
                .get()
                .uri("/api/assign/escalations/manager/m1")
                .exchange()
                .expectStatus().isOk();
    }
}
