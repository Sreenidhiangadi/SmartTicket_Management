...
class AgentClientServiceImplTest {
	...
	void fetchActiveAgents_success() {
		...
		when(responseSpec.bodyToFlux(AgentDto.class)).thenReturn(Flux.just(agent));

		StepVerifier.create(service.fetchActiveAgents()).expectNextMatches(a -> a.getId().equals("a1") && a.isActive())
				.verifyComplete();
	}
}
