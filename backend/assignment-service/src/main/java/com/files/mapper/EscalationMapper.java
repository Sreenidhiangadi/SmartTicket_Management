...
public final class EscalationMapper {
	...
	public static EscalationResponse toResponse(EscalationLog log) {
		return EscalationResponse.builder().ticketId(log.getTicketId()).agentId(log.getAgentId())
				.escalatedToManagerId(log.getEscalatedToManagerId()).reason(log.getReason())
				.escalatedAt(log.getEscalatedAt()).build();
	}
}
