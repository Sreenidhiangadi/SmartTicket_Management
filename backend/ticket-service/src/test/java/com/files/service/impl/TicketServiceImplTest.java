...
class TicketServiceImplTest {
    ...
    void getTicketById_success() {
        ...
        StepVerifier.create(service.getTicketById("t1"))
                .expectNextCount(1)
                .verifyComplete();
    }
    ...
    void getTicketById_notFound() {
        when(ticketRepository.findById("x"))
                .thenReturn(Mono.empty());

        StepVerifier.create(service.getTicketById("x"))
    ...
    void assignTicket_success() {
        ...
        when(ticketHistoryService.record(any(), any(), any(), any()))
                .thenReturn(Mono.empty());

        StepVerifier.create(service.assignTicket("t1", "agent2"))
                .expectNextCount(1)
                .verifyComplete();
    }
    ...
    void updateStatus_success() {
        ...
        StepVerifier.create(
                        service.updateStatus("t1", TicketStatus.IN_PROGRESS)
                                .contextWrite(
    ...
    void getTickets_userBranch() {
        ...
        when(ticketRepository.findByCreatedBy("user1"))
                .thenReturn(Flux.just(ticket));

        TestingAuthenticationToken auth =
                new TestingAuthenticationToken("user1", null, "ROLE_USER");

        StepVerifier.create(
                service.getTickets(null, null, 0, 10)
                        .contextWrite(
                                ReactiveSecurityContextHolder.withAuthentication(auth)
                        )
        )
    ...
    void getTickets_agentBranch() {
        ...
        when(ticketRepository.findByAssignedTo("agent1"))
                .thenReturn(Flux.just(ticket));

        TestingAuthenticationToken auth =
                new TestingAuthenticationToken("agent1", null, "ROLE_AGENT");
    ...
    void slaBreaches_success() {
        ...
        when(ticketRepository.findBySlaBreachedTrue())
                .thenReturn(Flux.just(ticket));
    ...
    void getTimeline_success() {
        ...
        when(ticketHistoryService.getHistory("t1"))
                .thenReturn(Flux.empty());

        when(commentService.getComments("t1"))
                .thenReturn(Flux.empty());

        StepVerifier.create(service.getTimeline("t1"))
                .verifyComplete();
    }
    ...
    void createTicket_autoAssignFails_shouldStillCreateTicket() {
        ...
        JwtAuthenticationToken auth =
                new JwtAuthenticationToken(jwt, Set.of(() -> "ROLE_USER"));
    ...
    void getTicketsByUser_success() {
        ...
        StepVerifier.create(service.getTicketsByUser("user1"))
                .expectNextCount(1)
                .verifyComplete();
    }
    @Test
    void getTicketsByAgent_success() {
        ...
        StepVerifier.create(service.getTicketsByAgent("agent1"))
                .expectNextCount(1)
                .verifyComplete();
    }
    @Test
    void getTicketsByStatus_success() {
        when(ticketRepository.findByStatus(TicketStatus.CREATED))
                .thenReturn(Flux.just(ticket));

        StepVerifier.create(service.getTicketsByStatus(TicketStatus.CREATED))
                .expectNextCount(1)
                .verifyComplete();
    }
    @Test
    ...
    void updateStatus_resolvedAfterSla_marksBreached() {
        ...
        JwtAuthenticationToken auth =
                new JwtAuthenticationToken(jwt, Set.of(() -> "ROLE_AGENT"));

        StepVerifier.create(
                service.updateStatus("t1", TicketStatus.RESOLVED)
                        .contextWrite(
                                ReactiveSecurityContextHolder.withAuthentication(auth)
                        )
        )
        .assertNext(r -> assertTrue(r.slaBreached()))
        .verifyComplete();
    }
    ...
    private boolean invokeTransition(TicketStatus from, TicketStatus to) {
        try {
            var m = TicketServiceImpl.class
                    .getDeclaredMethod("isValidTransition", TicketStatus.class, TicketStatus.class);
    ...
    void markSlaBreached_success() {
        ...
        StepVerifier.create(service.markSlaBreached("t1"))
                .verifyComplete();
    }
}
