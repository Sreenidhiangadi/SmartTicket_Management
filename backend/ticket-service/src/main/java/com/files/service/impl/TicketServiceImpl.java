...
public class TicketServiceImpl implements TicketService {
	...
	public Mono<TicketResponse> createTicket(CreateTicketRequest request) {
		...
		return ReactiveSecurityContextHolder.getContext().map(ctx -> (JwtAuthenticationToken) ctx.getAuthentication())
				.flatMap(jwtAuth -> {
					...
					Instant slaDueAt = now.plus(SlaPolicy.getDuration(request.priority()));

					Ticket ticket = Ticket.builder().title(request.title()).description(request.description())
							.category(request.category()).priority(request.priority()).status(TicketStatus.CREATED)
							.createdBy(userId).createdByEmail(email).createdAt(now).updatedAt(now).slaDueAt(slaDueAt)
							.slaBreached(false).build();

					return ticketRepository.save(ticket).doOnSuccess(saved -> {
						...
						kafkaTemplate.send("ticket-created-events", saved.getId(), event);
					}).flatMap(saved -> ticketHistoryService.record(saved.getId(), TicketHistoryAction.CREATED, userId,
							"Ticket created (SLA due at " + slaDueAt + ")").thenReturn(saved))

							.flatMap(saved -> assignmentWebClient.post()
									.uri(uriBuilder -> uriBuilder.path("/api/assign/auto/{ticketId}")
											.queryParam("priority", saved.getPriority().name()).build(saved.getId()))
									.header("Authorization", bearerToken).retrieve()
									.bodyToMono(AutoAssignmentResponse.class).flatMap(resp -> {
										saved.setAssignedTo(resp.getAgentId());
										saved.setStatus(TicketStatus.ASSIGNED);
										saved.setUpdatedAt(Instant.now());

										return ticketRepository.save(saved).doOnSuccess(updated -> {
										...
										}).flatMap(updated -> ticketHistoryService
												.record(updated.getId(), TicketHistoryAction.ASSIGNED,
														resp.getAgentId(),
	...
	public Mono<TicketResponse> getTicketById(String id) {
		return ticketRepository.findById(id).switchIfEmpty(Mono.error(new TicketNotFoundException(id)))
				.map(this::toResponse);
	}

	@Override
	public Flux<TicketResponse> getTicketsByUser(String userId) {
		return ticketRepository.findByCreatedBy(userId).map(this::toResponse);
	}

	@Override
	public Flux<TicketResponse> getTicketsByAgent(String agentId) {
		return ticketRepository.findByAssignedTo(agentId).map(this::toResponse);
	}

	@Override
	public Flux<TicketResponse> getTicketsByStatus(TicketStatus status) {
		return ticketRepository.findByStatus(status).map(this::toResponse);
	}

	@Override
	public Mono<TicketResponse> assignTicket(String ticketId, String agentId) {
		return ticketRepository.findById(ticketId).switchIfEmpty(Mono.error(new TicketNotFoundException(ticketId)))
				.flatMap(ticket -> {

					if (ticket.getStatus() == TicketStatus.CLOSED || ticket.getStatus() == TicketStatus.CANCELLED) {
						return Mono
								.error(new IllegalStateException("Closed or cancelled tickets cannot be reassigned"));
					}

					ticket.setAssignedTo(agentId);
					ticket.setStatus(TicketStatus.ASSIGNED);
					ticket.setUpdatedAt(Instant.now());
					ticket.setSlaDueAt(Instant.now().plus(SlaPolicy.getDuration(ticket.getPriority())));
					ticket.setSlaBreached(false);
					return ticketRepository.save(ticket)
							.flatMap(saved -> ticketHistoryService.record(saved.getId(), TicketHistoryAction.ASSIGNED,
									agentId, "Ticket assigned to agent " + agentId).thenReturn(saved));
				}).map(this::toResponse);
	}
	...
	public Mono<TicketResponse> updateStatus(String ticketId, TicketStatus newStatus) {
		...
		return ReactiveSecurityContextHolder.getContext().map(ctx -> (JwtAuthenticationToken) ctx.getAuthentication())
				.flatMap(jwtAuth -> {
					...
					Set<String> roles = jwtAuth.getAuthorities().stream().map(GrantedAuthority::getAuthority)
							.map(r -> r.replace("ROLE_", "")).collect(Collectors.toSet());

					return ticketRepository.findById(ticketId)
							.switchIfEmpty(Mono.error(new TicketNotFoundException(ticketId))).flatMap(ticket -> {

								if (ticket.getStatus() == TicketStatus.CLOSED
										|| ticket.getStatus() == TicketStatus.CANCELLED) {
									return Mono.error(
											new IllegalStateException("Closed or Cancelled tickets cannot be updated"));
								}

								if (roles.contains("AGENT") && !roles.contains("MANAGER") && !roles.contains("ADMIN")
										&& !userId.equals(ticket.getAssignedTo())) {
									return Mono
											.error(new IllegalStateException("Agent can update only assigned tickets"));
								}

								if (!isValidTransition(ticket.getStatus(), newStatus)) {
									return Mono.error(new IllegalStateException(
											"Invalid status transition: " + ticket.getStatus() + " → " + newStatus));
								}
								...
								if (newStatus == TicketStatus.RESOLVED && ticket.getSlaDueAt() != null
								...
								return ticketRepository.save(ticket).doOnSuccess(saved -> {
									...
									kafkaTemplate.send("ticket-status-events", saved.getId(), event);
								}).flatMap(saved -> ticketHistoryService.record(saved.getId(),
										TicketHistoryAction.STATUS_CHANGED, userId, oldStatus + " → " + newStatus)
										.thenReturn(saved));
							}).map(this::toResponse);
				});
	}
	...
	public Mono<TicketResponse> reopenTicket(String ticketId) {
		return withAuthContextMono((userId, roles) -> ticketRepository.findById(ticketId)
				.switchIfEmpty(Mono.error(new TicketNotFoundException(ticketId))).flatMap(ticket -> {

					if (ticket.getStatus() != TicketStatus.RESOLVED && ticket.getStatus() != TicketStatus.CLOSED) {
						return Mono.error(new IllegalStateException("Only RESOLVED or CLOSED tickets can be reopened"));
					}
					...
					return ticketRepository.save(ticket)
							.flatMap(saved -> ticketHistoryService
									.record(saved.getId(), TicketHistoryAction.REOPENED, userId, "Ticket reopened")
									.thenReturn(saved));
				}).map(this::toResponse));
	}
	...
	public Mono<TicketResponse> cancelTicket(String ticketId) {
		return withAuthContextMono((userId, roles) -> ticketRepository.findById(ticketId)
				.switchIfEmpty(Mono.error(new TicketNotFoundException(ticketId))).flatMap(ticket -> {
					...
					if (ticket.getStatus() == TicketStatus.RESOLVED || ticket.getStatus() == TicketStatus.CLOSED) {
						return Mono.error(new IllegalStateException("Resolved or Closed tickets cannot be cancelled"));
					}

					if (roles.contains("USER") && !roles.contains("ADMIN") && !userId.equals(ticket.getCreatedBy())) {
						return Mono.error(new IllegalStateException("User can cancel only their own ticket"));
					}
					...
					return ticketRepository.save(ticket)
							.flatMap(saved -> ticketHistoryService
									.record(saved.getId(), TicketHistoryAction.CANCELLED, userId, "Ticket cancelled")
									.thenReturn(saved));
				}).map(this::toResponse));
	}

	@Override
	public Flux<TicketResponse> getTickets(TicketStatus status, TicketPriority priority, int page, int size) {
		...
		return withAuthContextFlux((userId, roles) -> {
			...
			return flux.filter(t -> status == null || t.getStatus() == status)
					.filter(t -> priority == null || t.getPriority() == priority).skip((long) page * size).take(size)
					.map(this::toResponse);
		});
	}
	...
	public Flux<TimelineItemResponse> getTimeline(String ticketId) {
		...
		Flux<TimelineItemResponse> historyFlux = ticketHistoryService.getHistory(ticketId).map(this::fromHistory);

		Flux<TimelineItemResponse> commentFlux = commentService.getComments(ticketId)
				.map(comment -> new TimelineItemResponse("COMMENT", "COMMENT", comment.commentedBy(), comment.comment(),
						comment.commentedAt()));

		return Flux.merge(historyFlux, commentFlux).sort(Comparator.comparing(TimelineItemResponse::timestamp));
	}
	...
	public Flux<SlaBreachReport> slaBreaches() {
		...
		return ticketRepository.findBySlaBreachedTrue()
				.map(ticket -> new SlaBreachReport(ticket.getId(), ticket.getSlaDueAt(), ticket.getStatus().name()));
	}
	...
	public Mono<Ticket> autoAssignTicket(String ticketId) {
		...
		return ReactiveSecurityContextHolder.getContext().map(ctx -> (JwtAuthenticationToken) ctx.getAuthentication())
				.flatMap(jwtAuth -> {
					...
					return ticketRepository.findById(ticketId)
							.switchIfEmpty(Mono.error(new IllegalStateException("Ticket not found")))
							.flatMap(ticket -> {

								if (ticket.getStatus() != TicketStatus.CREATED) {
									return Mono.error(
											new IllegalStateException("Only CREATED tickets can be auto-assigned"));
								}

								if (ticket.getAssignedTo() != null) {
									log.info("Ticket {} already assigned to {}, skipping auto-assign", ticketId,
								...
								return assignmentWebClient.post().uri("/api/assign/auto/{ticketId}", ticketId)
										.header("Authorization", bearerToken).retrieve()
										.onStatus(status -> status.is4xxClientError() || status.is5xxServerError(),
												response -> response.bodyToMono(String.class).flatMap(body -> {
													log.error("Auto-assign failed for ticket {}: {}", ticketId, body);
													return Mono.error(
															new IllegalStateException("Auto-assign service failed"));
												}))
										.bodyToMono(AutoAssignmentResponse.class).flatMap(response -> {
											...
											return ticketRepository.save(ticket)
													.flatMap(
															saved -> ticketHistoryService
																	.record(saved.getId(), TicketHistoryAction.ASSIGNED,
																			response.getAgentId(),
	...
	private TimelineItemResponse fromHistory(TicketHistory history) {
		return new TimelineItemResponse("HISTORY", history.getAction().name(), history.getPerformedBy(),
				history.getDescription(), history.getCreatedAt());
	}
	...
	private Flux<TicketResponse> withAuthContextFlux(AuthFluxHandler handler) {
		return ReactiveSecurityContextHolder.getContext().map(ctx -> ctx.getAuthentication()).flatMapMany(auth -> {
			String userId = auth.getName();
			Set<String> roles = auth.getAuthorities().stream().map(GrantedAuthority::getAuthority)
					.map(r -> r.replace("ROLE_", "")).collect(Collectors.toSet());
			return handler.handle(userId, roles);
	...
	private TicketResponse toResponse(Ticket ticket) {
		return new TicketResponse(ticket.getId(), ticket.getTitle(), ticket.getDescription(), ticket.getCategory(),
				ticket.getPriority(), ticket.getStatus(), ticket.getCreatedBy(), ticket.getAssignedTo(),
				ticket.getCreatedAt(), ticket.getUpdatedAt(), ticket.getResolvedAt(), ticket.getClosedAt(),
				ticket.getSlaDueAt(), ticket.isSlaBreached());
	}
}
